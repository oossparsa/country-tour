...
def start():
    ...
    #u = int (0.3 * p)
    u = int (input('number of parents to be selected ? '))
    l = p-u
    preparefile()
    fitness=[0]
    iteration = 0
...
def preparefile():
    file = open(path + "results-Max.txt", 'w')
    file.write("Maximization\n#\t\tMax\t\tMean\t\tmin\n\n")
    file.close()
...
def IsFinished(iteration , fitness):
    if max(fitness) > 200000 or iteration>1000:
        return True
    else:
        return False
